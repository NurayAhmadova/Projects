...
FORM_URL = "https://docs.google.com/forms/d/e/1FAIpQLScspvr2tfcuRxnTGelNpifJwVHbBOnAJTAOFqeIAC-sb_PYNw/viewform"
...
def get_all_leases():
    ...
    for n in range(len(lease_list)):
        address = lease_list[n](".list-card-addr").getText()
        print(address)
        price = lease_list[n](".list-card-price").getText().split("/")[0].split("+")[0].replace("$", "").split(" ")[0]
        link = lease_list[n](".list-card-link").get("href")
        # get absolute path
...
# Fill out the form.(Japanese site)
def input_form(lease_dic):
    ...
    for lease in lease_dic.values():
        ...
        time.sleep(2)
        driver.find_element_by_link_text("別の回答を送信").click()
